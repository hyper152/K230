...
# PipeLine类
class PipeLine:
    def __init__(self,rgb888p_size=[224,224],display_mode="hdmi",display_size=None,
                 osd_layer_num=1,debug_mode=0,gray_size=None,
                 gray_channel=CAM_CHN_ID_1):
        # sensor给AI的图像分辨率
    ...
    def create(self,sensor=None,hmirror=None,vflip=None,fps=60):
        with ScopedTiming("init PipeLine",self.debug_mode > 0):
            ...
            # Channel 1 is generated by VICAP hardware for wireless image
            # transmission. This avoids Python RGB-to-gray conversion/resizing.
            if self.gray_size is not None:
                gray_w = ALIGN_UP(self.gray_size[0], 16)
                gray_h = self.gray_size[1]
                self.sensor.set_framesize(w=gray_w, h=gray_h,
                                          chn=self.gray_channel)
                self.sensor.set_pixformat(Sensor.GRAYSCALE,
                                          chn=self.gray_channel)
                self.gray_size = [gray_w, gray_h]
                print("PipeLine gray channel: ch{} {}x{}".format(
                    self.gray_channel, gray_w, gray_h))
